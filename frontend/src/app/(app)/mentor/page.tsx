  ...
  return (
    <div className="p-6 sm:p-10 max-w-7xl mx-auto w-full space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-border-dark">
        ...
        <div className="flex items-center gap-1.5 bg-surface-dark p-1 rounded-[12px] border border-border-dark self-start md:self-auto">
          <button
          ...
          >
            ...
            {stats?.pendingQueueCount > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-slate-900 text-accent-400 font-mono text-[10px] font-bold">
          ...
          >
            ...
            {stats?.openThreadsCount > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-danger-600 text-white font-mono text-[10px] font-bold">
          ...
          >
            ...
            <span className="px-1.5 py-0.2 rounded-full bg-surface-raised text-slate-300 font-mono text-[10px]">
              {mentees?.length || 0}
            </span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-5">
            <div className="w-9 h-9 rounded-full bg-accent-500/10 border border-accent-500/25 flex items-center justify-center text-accent-400 mb-2">
              <Eye className="w-5 h-5" />
            </div>
            <p className="text-xs text-slate-400 font-body">Pending Reviews</p>
            <p className="font-mono text-2xl font-bold text-white mt-1 tabular-nums">{stats?.pendingQueueCount || 0}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-slate-400 font-body">Approved Projects</p>
            <p className="font-mono text-2xl font-bold text-accent-400 mt-1 tabular-nums">{stats?.approvedReviewsCount || 0}</p>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="p-5">
            <div className="w-9 h-9 rounded-full bg-primary-500/10 border border-primary-500/25 flex items-center justify-center text-primary-400 mb-2">
              <MessageSquare className="w-5 h-5" />
            </div>
            <p className="text-xs text-slate-400 font-body">Open Questions</p>
            <p className="font-mono text-2xl font-bold text-white mt-1 tabular-nums">{stats?.openThreadsCount || 0}</p>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="p-5">
            <div className="w-9 h-9 rounded-full bg-gold-500/10 border border-gold-500/25 flex items-center justify-center text-gold-400 mb-2">
              <Users className="w-5 h-5" />
            </div>
            <p className="text-xs text-slate-400 font-body">Active Mentees</p>
            <p className="font-mono text-2xl font-bold text-white mt-1 tabular-nums">{mentees?.length || 0}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="space-y-6">
          ...
          {queue?.length === 0 ? (
            <Card className="p-10 text-center space-y-3">
              <CheckCircle2 className="w-10 h-10 text-accent-400 mx-auto" />
            ...
            <Card className="divide-y divide-border-dark overflow-hidden">
              {queue?.map((item: any) => (
                <div
                  key={item.id}
                  className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-surface-raised/40 transition-colors"
                >
                  <div className="flex items-start gap-4">
                    <Avatar name={item.user?.name || 'Student'} size="md" variant="student" />
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm text-white">{item.mission?.title}</span>
                        <Badge
                      ...
                      <p className="text-xs text-slate-400 font-mono">
                        Student: <strong className="text-slate-200">{item.user?.name}</strong> ·{' '}
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-[6px] text-xs font-semibold ${
                    inquiryFilter === filter
                      ...
                      : 'bg-surface-dark text-slate-400 border border-border-dark'
                  }`}
                >
                ...
                <div
                  ...
                  className={`p-3.5 rounded-[10px] border cursor-pointer transition-all ${
                    activeThread?.id === t.id
                      ...
                      : 'border-border-dark bg-surface-dark hover:bg-surface-raised'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="font-semibold text-xs text-white truncate">{t.subject}</span>
                    <Badge variant={t.status === 'RESOLVED' ? 'verified' : 'streak'} size="sm">
                      {t.status}
                    </Badge>
                  </div>
                  <p className="text-xs text-slate-400 line-clamp-1">{t.messages?.[0]?.content}</p>
                  <p className="text-[10px] text-slate-500 font-mono mt-1">From: {t.student?.name}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="md:col-span-8">
            {activeThread ? (
              <Card className="h-full flex flex-col justify-between">
                <CardHeader className="border-b border-border-dark pb-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Student: <strong className="text-white">{activeThread.student?.name}</strong> ·{' '}
                    ...
                    <div
                      ...
                      className={`p-3.5 rounded-[10px] text-xs space-y-2 ${
                        msg.senderRole === 'MENTOR'
                          ? 'bg-accent-950/20 border border-accent-500/30 ml-6'
                          : 'bg-night border border-border-dark mr-6'
                      }`}
                    >
                      <div className="flex items-center justify-between font-mono text-[11px] text-slate-400">
                        <span className="font-semibold text-white">{msg.senderName} ({msg.senderRole})</span>
                        <span>{new Date(msg.createdAt).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-slate-200 leading-relaxed font-body">{msg.content}</p>
                      {msg.codeSnippet && (
                        <pre className="p-3 bg-night rounded border border-border-dark font-mono text-[11px] text-accent-300 overflow-x-auto">
                ...
                {/* Reply Form */}
                <div className="p-4 border-t border-border-dark bg-night/50 space-y-3">
                  <textarea
                    rows={2}
                    ...
                    className="w-full bg-surface-dark border border-border-dark rounded-[10px] p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-accent-500 font-body"
                  />
                  ...
                  {showReplyCodeInput && (
                    <textarea
                      ...
                      className="w-full bg-night border border-border-dark rounded-[10px] p-3 text-xs font-mono text-accent-300 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-accent-500"
                    />
                  )}
              ...
              <Card className="h-full flex items-center justify-center p-10 text-slate-500 text-xs font-body">
            ...
            <div key={student.id} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-accent-400 font-mono">{student.trackName || 'Frontend Web Development'}</p>
                  <p className="text-xs text-slate-400 font-mono">Streak: {student.streak || 0}d · XP: {student.xp || 0}</p>
                </div>
              </div>
      ...
      {activeSubmission && (
        <div className="fixed inset-0 z-50 bg-night/85 backdrop-blur-md flex items-center justify-center p-4">
          <Card className="max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-slide-up border-accent-500/30">
            <CardHeader className="border-b border-border-dark flex flex-row items-center justify-between pb-4">
              <div>
                <CardTitle className="text-lg font-bold">
                  Code Evaluation: {activeSubmission.mission?.title}
                </CardTitle>
                <CardDescription>
                  Student: <strong className="text-white">{activeSubmission.user?.name}</strong> ·{' '}
            ...
            <CardContent className="p-6 space-y-5 flex-1 overflow-y-auto">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between border-b border-border-dark pb-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-[6px] text-xs font-mono uppercase font-semibold ${
                          codeTab === tab
                            ...
                            : 'bg-night text-slate-400 border border-border-dark hover:text-white'
                        }`}
                      >
                        {tab}
                      </button>
                  ...
                  <span className="text-xs text-slate-500 font-mono">AST DOM Validated</span>
                </div>

                <pre className="p-4 bg-night rounded-[10px] border border-border-dark font-mono text-xs text-slate-200 overflow-x-auto max-h-56">
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-white">Mentor Rating & Endorsement</label>
                    ...
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        ...
                        className={`w-4 h-4 cursor-pointer transition-colors ${
                          star <= reviewRating ? 'text-gold-400 fill-gold-400' : 'text-slate-600'
                        }`}
                      />
                    ))}
                  </div>
                ...
                {/* Templates */}
                <div className="space-y-1">
                  <span className="text-[11px] text-slate-400">Quick endorsement template:</span>
                      ...
                      <button
                        ...
                        className={`p-2 rounded text-left text-[11px] border transition-all truncate ${
                          feedbackText === tmpl
                            ...
                            : 'border-border-dark bg-night text-slate-400 hover:text-white'
                        }`}
                      >
                        {tmpl}
                      </button>
                ...
                <textarea
                  ...
                  className="w-full bg-night border border-border-dark rounded-[10px] p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-accent-500 font-body"
                />
              </div>
